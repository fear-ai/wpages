...
class TestPagesFocus(unittest.TestCase):
    def test_focuslist_dedupe_nocase(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "pages.list"
            path.write_text("Home, home\nAbout\n", encoding="utf-8")
            result = load_focus_list(path, case_sensitive=False)
            self.assertEqual([entry.name for entry in result.entries], ["Home", "About"])
            self.assertEqual([entry.key for entry in result.entries], ["home", "about"])
            self.assertEqual(result.duplicates, ["home"])

    def test_focuslist_case(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "pages.list"
            path.write_text("Home\nhome\n", encoding="utf-8")
            result = load_focus_list(path, case_sensitive=True)
            self.assertEqual([entry.name for entry in result.entries], ["Home", "home"])
            self.assertEqual([entry.key for entry in result.entries], ["Home", "home"])
            self.assertEqual(result.duplicates, [])

    def test_rowkeys_nocase(self) -> None:
        rows = parse_dump(TESTS_DIR / "sample.out").rows
        rows_with_keys = build_rows_keys(rows, case_sensitive=False)
    ...
    def test_focusmatch_exact_best(self) -> None:
        rows = parse_dump(TESTS_DIR / "sample.out").rows
        title_index = build_title_index(rows, case_sensitive=True)
        rows_with_keys = build_rows_keys(rows, case_sensitive=True)
    ...
    def test_focusmatch_prefix_best(self) -> None:
        rows = parse_dump(TESTS_DIR / "sample.out").rows
    ...
    def test_label_exact_first(self) -> None:
        rows = parse_dump(TESTS_DIR / "sample.out").rows
    ...
    def test_label_longest_prefix(self) -> None:
        rows = parse_dump(TESTS_DIR / "sample.out").rows
        row = next(r for r in rows if r.title == "Contact")
        entries = [
            FocusEntry(name="Con", key="Con", key_len=3),
